...
describe 'University Profiles API', swagger_doc: 'v1/swagger.json' do
  path '/university/profile' do
    get 'Gets the current user profile' do
      response '200', 'profile returned' do
        schema '$ref' => '#/definitions/university_profile'
        tags 'Profiles'

        let('Authorization') { "Bearer #{auth[:token]}" }

        before { create(:university_profile_with_images, user: auth[:user]) }
    ...
    patch 'Updates the current user profile' do
      ...
      response '200', 'profile updated' do
        ...
        let(:profile) {{ profile: attributes_for(:updated_university_profile) }}
        ...
        it 'updates the name' do |example|
          expect(json[:name]).to eq(
            attributes_for(:updated_university_profile)[:name]
          )
        end

        it 'updates the contact person' do |example|
          expect(json[:contact_person]).to eq(
            attributes_for(:updated_university_profile)[:contact_person]
          )
        end

        it 'updates the industry sector' do |example|
          expect(json[:industry_sector]).to eq(
            attributes_for(:updated_university_profile)[:industry_sector]
          )
        end

        it 'updates the number of employees' do |example|
          expect(json[:number_of_employees]).to eq(
            attributes_for(:updated_university_profile)[:number_of_employees]
          )
        end

        it 'updates the country' do |example|
          expect(json[:country]).to eq(
            attributes_for(:updated_university_profile)[:country]
          )
        end

        it 'updates the city' do |example|
          expect(json[:city]).to eq(
            attributes_for(:updated_university_profile)[:city]
          )
        end

        it 'updates the street' do |example|
          expect(json[:street]).to eq(
            attributes_for(:updated_university_profile)[:street]
          )
        end

        it 'updates the What we do' do |example|
          expect(json[:what_we_do]).to eq(
            attributes_for(:updated_university_profile)[:what_we_do]
          )
        end

        it 'updates the Why we do it' do |example|
          expect(json[:why_we_do_it]).to eq(
            attributes_for(:updated_university_profile)[:why_we_do_it]
          )
        end
        ...
        it 'updates the Special features' do |example|
          expect(json[:special_features]).to eq(
            attributes_for(:updated_university_profile)[:special_features]
          )
        end

        it 'updates the Home Page' do |example|
          expect(json[:home_page]).to eq(
            attributes_for(:updated_university_profile)[:home_page]
          )
        end

        it 'updates the Youtube Link' do |example|
          expect(json[:youtube_link]).to eq(
            attributes_for(:updated_university_profile)[:youtube_link]
          )
        end

        it 'updates the Facebook Link' do |example|
          expect(json[:facebok_link]).to eq(
            attributes_for(:updated_university_profile)[:facebok_link]
          )
        end

        it 'updates the Twitter Link' do |example|
          expect(json[:twitter_link]).to eq(
            attributes_for(:updated_university_profile)[:twitter_link]
          )
        end
      end
      ...
      response '400', 'invalid request' do
        ...
        #   let(:profile) {{ profile: 
        #     attributes_for(:university_profile, name: '') 
        #   }}
